...
public class MinecartLag implements Listener {
    ...
    public void onSpawn(VehicleCreateEvent event) {
        ...
        if (ammount >= max) {
            ...
            Utils.sendOpMessage(Utils.getPrefix() + "&6Removed &r&3" + chunk.getEntities().length + " " + formattedName
                    + "&r&6 from a lag machine owned by&r&3 " + player.getName());
            System.out.println(ChatColor.translateAlternateColorCodes('&',
                    Utils.getPrefix() + "&6Removed &r&3" + chunk.getEntities().length + " " + formattedName
                            + "&r&6 from a lag machine owned by&r&3 " + player.getName()));
